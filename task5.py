...
for i, k in tqdm(enumerate(k_values), total=len(k_values), desc="UMAP Progress"):
    start_time = time.time()
    
    # 应用 UMAP 降维
    umap_model = umap.UMAP(n_components=2, n_neighbors=k, min_dist=min_dist, random_state=20241106)
...
# 遍历不同的最近邻数
with open(os.path.join(save_dir, f"UMAP Calculate time.txt"), 'w') as file:
    for i, k in enumerate(k_values):
        ax = axes[i // 3, i % 3]
        unique_labels = np.unique(labels_sample)
        for label in unique_labels:
            ax.scatter(results[k][labels_sample == label, 0], results[k][labels_sample == label, 1], label=str(label), s=10)
        ax.set_xlabel("UMAP componet1")
        ax.set_ylabel("UMAP componet1")
        ax.legend()
        ax.set_title(f"k:{k}")
...
fig.suptitle('UMAP of dataset with different k values', fontsize=16, y=1.03)
plt.savefig(os.path.join(save_dir,  "UMAP of dataset with different k values.png"))
plt.close()