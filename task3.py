...
for i, k in tqdm(enumerate(k_values), total=len(k_values), desc="LLE Progress"):
...
print(f"LLE time = {all_elapsed_time:.4f}s")

save_dir = f"./task3/time={all_elapsed_time:.4f}s"
...
# 遍历不同的最近邻数
with open(os.path.join(save_dir, f"LLE Calculate time.txt"), 'w') as file:
    for i, k in enumerate(k_values):
        ax = axes[i // 3, i % 3]
        unique_labels = np.unique(labels_sample)
        for label in unique_labels:
            ax.scatter(results[k][labels_sample == label, 0], results[k][labels_sample == label, 1], label=str(label), s=10)
        ax.set_xlabel("LLE componet1")
        ax.set_ylabel("LLE componet1")
        ax.legend()
        ax.set_title(f"k:{k}")
...
fig.suptitle('LLE of dataset with different k values', fontsize=16, y=1.03)
plt.savefig(os.path.join(save_dir,  "LLE of dataset with different k values.png"))
plt.close()